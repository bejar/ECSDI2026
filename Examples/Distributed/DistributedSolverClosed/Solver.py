                ...
                if len(param) == 4:
                    ...
                    if probtype in ["ARITH", "MFREQ"]:
                        if len(minions[probtype]) != 0:
                            ...
                            requests.get(
                                minionadd + "/message", params={"message": mess}
                            )
                            ...
                            if logger is not None:
                                try:
                                    requests.get(
                                        logger + "/message",
                                        params={"message": f"{solverid},{probtype}"},
                                        timeout=5,
                                    )
                                except Exception:
                                    pass
...
def info():
    ...
    return render_template(
        "solverproblems.html",
        probs=obscure(problems),
        minions=[(p, id, minions[p][id]) for p in minions for id in minions[p]],
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dir", default=None, help="Direccion del servicio de directorio"
    )
    ...
    if "OK" in resp:
        ...
        loggeradd = requests.get(
            diraddress + "/message", params={"message": "SEARCH|LOGGER"}
        ).text
        if "OK" in loggeradd:
            logger = loggeradd[4:]
